<!-- ##### PACKAGES PAGE ##### -->
	<section id="packageBanner" class="staticPageBannerSection">
		<div class="container">
			<div class="row">
				<article class="col-md-12">
					<div class="packagebannerHead">
						<h1>Subscription &amp; Packages</h1>
						<span>We have packages according your needs, so select one and increase your business.</span>
					</div>
				</article>
			</div>
		</div>
	</section>
	<section id="packageDescription">
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<div class="packageDescriptionSection">
						<p>
							Following packages for subscription are available kindly go through it to get high quality services through verified operators.
						</p>
						<ul class="nav nav-tabs packageTabs" role="tablist">
							<li role="presentation" class="active">
								<a href="#packAndMove" aria-controls="packAndMove" role="tab" data-toggle="tab">&nbsp;For Packers &amp; Movers &amp; Crane Provider
                                </a>
							</li>
							<li role="presentation">
								<a href="#transAndLogis" aria-controls="transAndLogis" role="tab" data-toggle="tab">&nbsp;For Transporter/Logistics</a>
							</li>
							<li role="presentation">
								<a href="#customer" aria-controls="customer" role="tab" data-toggle="tab">&nbsp;For Customers</a>
							</li>
						</ul>
						<div class="packageTabsContent">
							<div class="tab-content">
								<div role="tabpanel" class="tab-pane active" id="packAndMove">
									<table class="table packerPackage">
										<thead>
											<tr>
												<th>Package Feature</th>
												<th style="text-align: center;">Packers &amp; Movers</th>
												<th style="text-align: center;">For Tanker Provider</th>
											</tr>
										</thead>
										<tbody>
											<tr>
												<td><span>Registartion</span></td>
												<td><span>21 days trail free</span></td>
												<td><span>21 days trail free</span></td>
											</tr>
											<tr>
												<td>
													<p>Place Order</p>
													<ul class="list-unstyled">
														<li><span>For Packers &amp; Movers service</span></li>
														<li><span>For Crane Service</span></li>
														<li><span>For Transport /  Logistic Service</span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class ">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
											</tr>
											<tr>
												<td><span>Find Order</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr>
												<td><span>View &amp; Rate Order</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr>
												<td><span>Select Station</span></td>
												<td>
													<span>3 Station free</span>
													<span>Rs.500 for addition of station</span>
												</td>
												<td>
													<span>3 Station free</span>
													<span>Rs.500 for addition of station</span>
												</td>
											</tr>
											<tr>
												<td><span>SMS for order accept / Confirm</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr>
												<td><span>Email Message</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr class="no-row-colspan">
												<td colspan="3"></td>
											</tr>
											<tr>
												<td><span>Price</span></td>
												<td><span>5500</span></td>
												<td><span>4500</span></td>
											</tr>
											<tr>
												<td><span>Discount</span></td>
												<td><span>600</span></td>
												<td><span>600</span></td>
											</tr>
											<tr>
												<td><span>Offer Price</span></td>
												<td>
													<span>4900</span>
													<a href="javascript:void(0);">Pay Now</a>
												</td>
												<td>
													<span>3900</span>
													<a href="javascript:void(0);">Pay Now</a>
												</td>
											</tr>
										</tbody>
									</table>
								</div>
								<div role="tabpanel" class="tab-pane" id="transAndLogis">
                                	<div class="table-responsive">
									<table class="table transporterPackage">
										<thead>
											<tr>
												<th><span>Package Feature</span></th>
												<th><span>For Logistic/ Transport</span></th>
												<th><span>For Commission Agent</span></th>
												<th><span>For Container Supplier</span></th>
												<th><span>For Fleet Owner</span></th>
												<th><span>For Trolly Provider</span></th>
												<th><span>For Tanker Provider</span></th>
											</tr>
										</thead>
										<tbody>
											<tr>
												<td><span>Registration</span></td>
												<td><span>21 days trail free</span></td>
												<td><span>21 days trail free</span></td>
												<td><span>21 days trail free</span></td>
												<td><span>21 days trail free</span></td>
												<td><span>21 days trail free</span></td>
												<td><span>21 days trail free</span></td>
											</tr>
											<tr>
												<td>
													
                                                    <p>Place Order</p>
													<ul class="list-unstyled">
														<li><span>For Packers &amp; Movers service</span></li>
														<li><span>For Crane Service</span></li>
														<li><span>For Transport /  Logistic Service</span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
											</tr>
											<tr>
												<td><span>Find Order</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr>
												<td><span>View &amp; Rate Order</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr>
												<td><span>Select Station</span></td>
												<td>
													<span>3 Station Free</span>
													<span>Rs.500 add</span>
												</td>
												<td>
													<span>3 Station Free</span>
													<span>Rs.500 add</span>
												</td>
												<td>
													<span>3 Station Free</span>
													<span>Rs.500 add</span>
												</td>
												<td>
													<span>3 Station Free</span>
													<span>Rs.500 add</span>
												</td>
												<td>
													<span>3 Station Free</span>
													<span>Rs.500 add</span>
												</td>
												<td>
													<span>3 Station Free</span>
													<span>Rs.500 add</span>
												</td>
											</tr>
											<tr>
												<td><span>SMS for order Accept/Confirm</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr>
												<td><span>Email Message</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr class="no-row-colspan">
												<td colspan="7"></td>
											</tr>
											<tr>
												<td><span>Price</span></td>
												<td><span>7500</span></td>
												<td><span>4500</span></td>
												<td><span>5500</span></td>
												<td><span>4500</span></td>
												<td><span>5000</span></td>
												<td><span>5000</span></td>
											</tr>
											<tr>
												<td><span>Discount</span></td>
												<td><span>600</span></td>
												<td><span>200</span></td>
												<td><span>600</span></td>
												<td><span>200</span></td>
												<td><span>500</span></td>
												<td><span>500</span></td>
											</tr>
											<tr>
												<td>
													<span>Offer Price</span>
												</td>
												<td>
													<span>6900</span>
													<a href="javascript:void(0);">Pay Now</a>
												</td>
												<td>
													<span>4300</span>
													<a href="javascript:void(0);">Pay Now</a>
												</td>
												<td>
													<span>4900</span>
													<a href="javascript:void(0);">Pay Now</a>
												</td>
												<td>
													<span>4300</span>
													<a href="javascript:void(0);">Pay Now</a>
												</td>
												<td>
													<span>4500</span>
													<a href="javascript:void(0);">Pay Now</a>
												</td>
												<td>
													<span>4500</span>
													<a href="javascript:void(0);">Pay Now</a>
												</td>
											</tr>
										</tbody>
									</table>
                                    </div>
								</div>
								<div role="tabpanel" class="tab-pane" id="customer">
									<table class="table packerPackage">
										<thead>
											<tr>
												<th>Package Feature</th>
												<th style="text-align: center;">Customer</th>
											</tr>
										</thead>
										<tbody>
											<tr>
												<td><span>Registartion</span></td>
												<td><span>Free</span></td>
											</tr>
											<tr>
												<td>
													<p>Place Order</p>
													<ul class="list-unstyled">
														<li><span>For Packers &amp; Movers service</span></li>
														<li><span>For Crane Service</span></li>
														<li><span>For Transport /  Logistic Service</span></li>
													</ul>
												</td>
												<td>
													<ul class="list-unstyled margin-top-class">
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
														<li><span><i class="fa fa-check" aria-hidden="true"></i></span></li>
													</ul>
												</td>
											</tr>
											<!-- <tr>
												<td><span>Find Order</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr> -->
											<tr>
												<td><span>View Order</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<!-- <tr>
												<td><span>Select Station</span></td>
												<td>
													<span>3 Station free</span>
													<span>Rs.500 for addition of station</span>
												</td>
											</tr> -->
											<tr>
												<td><span>SMS for order accept / Confirm</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr>
												<td><span>Email Message</span></td>
												<td><span><i class="fa fa-check" aria-hidden="true"></i></span></td>
											</tr>
											<tr class="no-row-colspan">
												<td colspan="3"></td>
											</tr>
											<tr>
												<td><span>Price</span></td>
												<td><span>-</span></td>
											</tr>
											<tr>
												<td><span>Discount</span></td>
												<td><span>-</span></td>
											</tr>
											<tr>
												<td><span>Offer Price</span></td>
												<td>
													<span>-</span>
													<!-- <a href="javascript:void(0);">Pay Now</a> -->
												</td>
											</tr>
										</tbody>
									</table>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>